...
document.addEventListener('DOMContentLoaded', function() {
  ...
  initFooterFloatingShapes();
  

});

// Footer Floating Shapes Animation
function initFooterFloatingShapes() {
  const footer = document.querySelector('.footer');
  if (!footer) {
  ...
  console.log('Footer found, initializing floating shapes...');
  
  // Add staggered animation delays to existing shapes
  const shapes = footer.querySelectorAll('.geometric-shape');
  console.log('Found shapes:', shapes.length);
  
  if (shapes.length === 0) {
    console.log('No geometric shapes found in footer');
    return;
  }
  
  shapes.forEach((shape, index) => {
    shape.style.animationDelay = `${index * 2}s`;
    console.log(`Applied animation delay ${index * 2}s to shape ${index}`);
    
    // Add spin click effect and dynamic movement
    shape.addEventListener('click', function(e) {
      e.stopPropagation();
      console.log('Shape clicked:', this.className);
      this.classList.add('clicked');
      
      // Generate random position for dynamic movement
      const footer = document.querySelector('.footer');
      const footerRect = footer.getBoundingClientRect();
      const maxX = footerRect.width - 60; // Account for element size with padding
      const maxY = footerRect.height - 60;
      
      const randomX = Math.max(20, Math.min(maxX, Math.random() * maxX));
      const randomY = Math.max(20, Math.min(maxY, Math.random() * maxY));
      
      // Apply new position with ultra-smooth transition
      this.style.transition = 'all 1.5s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
      ...
      setTimeout(() => {
        ...
        console.log('Shape animation completed');
      }, 1500);
    });
    
    console.log(`Event listener added to shape ${index}`);
  });
  ...
  if (orbs.length === 0) {
    console.log('No gradient orbs found in footer');
    return;
  }
  
  orbs.forEach((orb, index) => {
    orb.style.animationDelay = `${index * 1.5}s`;
    console.log(`Applied animation delay ${index * 1.5}s to orb ${index}`);
    
    // Add spin click effect and dynamic movement
    orb.addEventListener('click', function(e) {
      e.stopPropagation();
      console.log('Orb clicked:', this.className);
      this.classList.add('clicked');
      ...
      setTimeout(() => {
        this.classList.remove('clicked');
        console.log('Orb animation completed');
      }, 1500);
    });
    
    console.log(`Event listener added to orb ${index}`);
  });
}
