...
function initPreloader() {
    ...
    function monitorDotPosition() {
        ...
        if (ball) {
            ...
            // Map animation progress to bar positions
            ...
            // Only update if we have a valid bar index and it's different
            if (newBarIndex !== -1 && newBarIndex !== currentBarIndex) {
                // Color previous bar white when dot leaves
                if (currentBarIndex >= 0 && currentBarIndex < bars.length) {
                    bars[currentBarIndex].style.background = '#ffffff';
                }
                
                // Color the new bar with dot color
    ...
    // Set up monitoring with higher frequency for better precision
    setInterval(monitorDotPosition, 50);
        ...
        return new Promise((resolve) => {
            ...
            const imagePromises = Array.from(images).map(img => {
                if (img.complete && img.naturalHeight !== 0) {
                    console.log(`Image already loaded: ${img.src || img.alt || 'unnamed'}`);
            ...
            // Check if all videos are fully loaded and ready to play
            ...
            const videoPromises = Array.from(videos).map(video => {
                return new Promise(resolve => {
                    ...
                    if (video.readyState >= 3) { // HAVE_FUTURE_DATA - enough data to play
                        console.log(`Video already ready: ${videoSrc}`);
                        resolve();
                    } else {
                        console.log(`Waiting for video to be ready: ${videoSrc}`);
                        // Listen for multiple events to ensure video is fully ready
                        const videoReady = () => {
            ...
            Promise.all([...imagePromises, ...videoPromises, ...resourcePromises, ...fontPromises])
                .then(() => {
                    console.log('All resources loaded, waiting additional time for stability...');
                    // Additional delay to ensure everything is stable and ready
                    setTimeout(() => {
    ...
    // Fallback: Complete loading after maximum 20 seconds if resources don't load
    setTimeout(() => {
