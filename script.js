...
window.addEventListener('load', function() {
    ...
    // Track preloader start time
    const preloaderStartTime = Date.now();
    console.log('Preloader started at:', preloaderStartTime);
    ...
    function showTextBasedOnLoadingTime() {
        ...
        // Show "Did You Know?" section 1 second after loading message (4 seconds total)
        if (elapsedTime >= 4000 && !didYouKnowSection.classList.contains('show')) {
        ...
        // Show facts section 1 second after "Did You Know?" (5 seconds total)
        if (elapsedTime >= 5000 && !factsSection.classList.contains('show')) {
            if (factsSection) {
        ...
        // Show skip button 1 second after facts (6 seconds total)
        if (elapsedTime >= 6000 && !skipButton.classList.contains('show')) {
            if (skipButton) {
                skipButton.classList.add('show');
                console.log('Skip button shown after', elapsedTime, 'ms of loading');
            }
        }
    }
    ...
    // Complete loading after 8 seconds or when everything is ready
    setTimeout(() => {
